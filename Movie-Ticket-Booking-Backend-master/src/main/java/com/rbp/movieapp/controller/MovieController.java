...
//    @PreAuthorize("hasRole('USER')")
	...
	public ResponseEntity<String> bookTickets(@RequestBody Ticket ticket, @PathVariable String movieName) {
		...
		Integer availableTicket = movieService.findAvailableTickets(movieName, ticket.getTheatreName()).get(0)
				.getNoOfTicketsAvailable();
		if (availableTicket >= ticket.getNoOfTickets()) {

			log.info("available tickets " + availableTicket);
	...
	@PutMapping("/{movieName}/update/{ticketsStatus}")
	@SecurityRequirement(name = "Bearer Authentication")
	...
	public ResponseEntity<String> updateTicketsStatus(@RequestBody Movie movie, @PathVariable String movieName,
			@PathVariable String ticketsStatus) {
		...
		if (availableMovies.isEmpty()) {
			...
			movieService.saveMovie(new Movie(objectId, movieName, movie.getTheatreName(),
					movie.getNoOfTicketsAvailable(), ticketsStatus));
